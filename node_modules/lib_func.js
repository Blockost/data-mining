...
var main = function(){
    ...
    generateClassesDependently(tab_xi, tab_mu, tab_sigma, tab_y, tab_y_13);
 ...
 * @param tab_sigma vecteur des variances
 * @param tab_y vecteur des classes
 * @param tab_y_13 vecteur des classes
 */
function generateClassesDependently(tab_xi, tab_mu, tab_sigma, tab_y, tab_y_13){
    /* Generate classes from math formula */
    for(var i = 0; i < n_lines; i++){
        tab_y[i] = 0;
        for(var j = 0; j < n_columns; j++) {
            tab_y[i] += (tab_xi[j][i]- tab_mu[j]) / tab_sigma[j];
        }
        tab_y[i] = Math.abs(tab_y[i]);
    }
