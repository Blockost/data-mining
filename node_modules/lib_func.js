...
/*    console.log("tab_xi :");
    ...
    console.log("Traitement à " + dependance)
    generateClasses_13(tab_y, tab_y_13);
    for (var i = Math.floor(dependance*n_lines); i < n_lines; i++){
        tab_y_comp[i - Math.floor(dependance*n_lines)] = classify(tab_xi.map(function(value, index){ return value[i]; }), tab_xi.slice(0, Math.floor(dependance*n_lines)), tab_y_13, 3);
    }
...
function displayErrorRatio(classes_defined, classes_found){
    ...
    console.log("Pourcentage d'erreur : " + total/classes_defined.length);
}