...
var main = function(data_read){
    ...
    if(typeof data_read === 'undefined'){
        data = generateData(number_of_data).map(function(element){
            return [element[0] * 118, element[1] * 37, element[2]];
        });
    ...
    // Sinon...
        ...
        if(data_read.filename == "Test-Clusters-Distincts.txt"){
            data = data_read.data.map(function(element){
                return [(element[0]+10) * 60, (element[1]+20) * 25, element[2]];
            });
        }else{
            data = data_read.data.map(function(element){
                return [(element[0]+5) * 125, (element[1]+5) * 60, element[2]];
            });
        }
...
var iterate = function(data, number_of_classes, barycenters){
    ...
    var inertie_intra_classes = getInertieIntraClasses(data, barycenters, barycenter_of_canvas, min_index);

    return [data, number_of_classes, barycenters, inertie_inter_classes / inertie_totale, inertie_intra_classes / inertie_totale];
};
...
/**
 * Calcul l'inertie d'une classe
 * @param vector vecteur de points de la classe
 * @param barycenter le barycentre de cette classe
 * @returns {number} l'inertie de la classe
 */
...
/**
 * Calcul l'inertie inter classes
 * @param data le jeu de données
 * @param barycenters l'ensemble des barycentres
 * @param barycenter_of_canvas le barycentre du canvas entier
 * @param min_index la liste des classes des points de @param data
 * @returns {number} l'inertie inter classes calculée
 */
...
/**
 * Calcul l'inertie intra classe
 * @returns {number}
 */
function getInertieIntraClasses(data, barycenters, barycenter_of_canvas, min_index){
    var inertie = 0;
    barycenters.forEach(function(element, index){
        inertie += countDuplicate(min_index, index)/data.length * getInertie(getPointsWithinClasses(data, index), element);
    });
    return inertie;
}

function getPointsWithinClasses(data, _class){
    var vector = [];
    data.forEach(function(element){
       if(element[2] == _class) vector.push(element);
    });
    return vector;
}
